...
export default async function HomePage() {
  ...
  const templeDescriptions: Record<string, string> = {
    "Erawan Shrine": "诚心祈愿的圣地。祈求健康、爱情与顺遂。",
    "Wat Pho": "曼谷最古老的寺庙之一，宁静而庄严。",
    "Wat Arun": "黎明之光，河畔古寺，心诚则灵。",
  };
  ...
  return (
    <>
          ...
          <div className="hero-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "var(--space-10)", alignItems: "center" }}>
            <div>
              <div style={{ marginBottom: "var(--space-4)" }}>
                <span className="badge badge-amber" style={{ fontSize: "var(--text-sm)", padding: "var(--space-1) var(--space-4)" }}>信众信赖，用心验证</span>
              </div>
              <h1 style={{ fontSize: "clamp(32px, 5vw, 52px)", fontWeight: 800, lineHeight: 1.1, marginBottom: "var(--space-5)" }}>
                您的祈愿，<br /><span style={{ color: "var(--amber-400)" }}>用心完成。</span>
              </h1>
              <p style={{ fontSize: "var(--text-lg)", color: "rgba(255,255,255,0.7)", marginBottom: "var(--space-6)", lineHeight: 1.7 }}>
                连接您与曼谷寺庙的本地执行人。每份订单附有照片与视频证明——让您安心知道，一切已妥善完成。
              </p>
              <div style={{ display: "flex", gap: "var(--space-3)", flexWrap: "wrap", marginBottom: "var(--space-6)" }}>
                <Link href="/temples" className="btn btn-primary btn-lg">浏览寺庙</Link>
                <Link href="/temple-request" className="btn btn-lg" style={{ background: "rgba(255,255,255,0.1)", color: "white", border: "1px solid rgba(255,255,255,0.2)" }}>申请寺庙</Link>
              </div>
              <div style={{ display: "flex", flexWrap: "wrap", gap: "var(--space-2)" }}>
                {[
                  { href: "/temples?intent=business", emoji: "💼", label: "事业" },
                  { href: "/temples?intent=health", emoji: "💗", label: "健康" },
                  { href: "/temples?intent=love", emoji: "💕", label: "感情" },
                  { href: "/temples?intent=luck", emoji: "🍀", label: "好运" },
                  { href: "/temples?intent=vow", emoji: "🌻", label: "还愿" },
                ].map((item) => (
            ...
            <div style={{ position: "relative", display: "flex", justifyContent: "center", alignItems: "center" }}>
              <img src="/josstick-hero.png" alt="燃烧的香" style={{ maxWidth: "100%", height: "auto", maxHeight: 420, objectFit: "contain", filter: "drop-shadow(0 0 30px rgba(184,134,11,0.25))" }} />
            </div>
          </div>
        </div>
      </section>
      ...
      <section style={{ padding: "var(--space-10) 0" }}>
        <div className="container">
          <div className="grid grid-4 text-center">
            {[
              { icon: "📸", title: "照片与视频证明", desc: "每份订单附有照片与视频，含专属验证码" },
              { icon: "🙏", title: "认证执行人", desc: "经过培训的本地执行人，熟悉每座寺庙的礼仪" },
              { icon: "🔒", title: "订单追踪", desc: "实时追踪您的订单，从下单到完成" },
              { icon: "📋", title: "标准流程保证", desc: "每项仪式均遵循团队核定的标准程序" },
            ].map((item) => (
      ...
      <section style={{ padding: "var(--space-10) 0", background: "var(--stone-50)" }}>
        <div className="container">
          <div style={{ textAlign: "center", marginBottom: "var(--space-8)" }}>
            <h2 style={{ fontSize: "var(--text-3xl)", fontWeight: 700, marginBottom: "var(--space-2)" }}>曼谷寺庙</h2>
            <p className="text-muted">信赖的寺庙，如法的仪式</p>
          </div>
          <div className="grid grid-3">
            {sortedTemples.map((temple) => {
              ...
              return (
                <Link key={temple.id} href={`/temples/${temple.id}`} className="card" style={{ overflow: "hidden", cursor: "pointer", textDecoration: "none", opacity: isActive ? 1 : 0.6 }}>
                  <div style={{ height: 180, background: isActive ? "linear-gradient(135deg, var(--earth-100), var(--amber-100))" : "var(--stone-100)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 56, position: "relative" }}>
                    {icon}
                    {!isActive && <span className="badge badge-orange" style={{ position: "absolute", top: "var(--space-3)", right: "var(--space-3)" }}>即将推出</span>}
                  </div>
                  <div className="card-body">
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "var(--space-1)" }}>
                      <div style={{ fontWeight: 700, fontSize: "var(--text-lg)" }}>{temple.name}</div>
                      <span className="badge badge-brown">{temple.verification_status === "verified" ? "已认证" : "待认证"}</span>
                    </div>
                    ...
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                      ...
                      {fromPrice !== undefined && <span style={{ fontWeight: 700, color: "var(--amber-700)" }}>RM{fromPrice} 起</span>}
                    </div>
                  </div>
                </Link>
              );
          ...
          <div className="text-center mt-6">
            <Link href="/temple-request" className="btn btn-secondary">找不到您的寺庙？申请加入 →</Link>
          </div>
        </div>
      </section>
      ...
      <section style={{ padding: "var(--space-10) 0" }}>
        <div className="container">
          <div style={{ textAlign: "center", marginBottom: "var(--space-8)" }}>
            <h2 style={{ fontSize: "var(--text-3xl)", fontWeight: 700, marginBottom: "var(--space-2)" }}>服务流程</h2>
            <p className="text-muted">简单、透明、恭敬</p>
          </div>
          <div className="grid grid-4 text-center">
            {[
              { num: "1️⃣", title: "选择", desc: "选择寺庙与配套，告诉我们您的祈愿。" },
              { num: "2️⃣", title: "付款", desc: "安全付款，完成后才释放款项。" },
              { num: "3️⃣", title: "执行", desc: "认证执行人前往寺庙完成仪式。" },
              { num: "4️⃣", title: "验证", desc: "收到照片、视频与专属验证码。" },
            ].map((step) => (
      ...
      <section style={{ background: "linear-gradient(135deg, var(--amber-700), var(--amber-600))", padding: "var(--space-10) 0", textAlign: "center", color: "white" }}>
        <div className="container">
          <h2 style={{ fontSize: "var(--text-3xl)", fontWeight: 700, marginBottom: "var(--space-3)" }}>准备好了吗？</h2>
          <p style={{ color: "rgba(255,255,255,0.8)", marginBottom: "var(--space-6)", fontSize: "var(--text-lg)" }}>浏览寺庙，诚心祈愿。</p>
          <Link href="/temples" className="btn" style={{ background: "white", color: "var(--amber-800)", fontWeight: 700, padding: "var(--space-4) var(--space-8)" }}>浏览寺庙</Link>
        </div>
      </section>
    </>
  );
