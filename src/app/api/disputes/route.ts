...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data, error } = await supabaseAdmin.from("disputes").insert(safeBody).select().single();
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const { data, error } = await supabaseAdmin.from("disputes").update(updates).eq("id", id).select().single();
    if (error) return NextResponse.json({ error: error.message }, { status: 400 });
    return NextResponse.json(data);
  } catch (err) {
    console.error("PATCH /api/disputes error:", err);
    return NextResponse.json({ error: "Internal server error" }, { status: 500 });
  }
}
